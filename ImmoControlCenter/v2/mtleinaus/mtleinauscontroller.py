...
from PySide6.QtWidgets import QDialog, QMenu, QApplication, QMessageBox
...
class MtlEinAusController( IccController ):
    ...
    def onShowZahlungsdetails( self, dummy ):
        ...
        mobj_id = model.getMietobjekt(indexes[0].row())
        ...
        eamodel = self.getEinzelzahlungenModelMonat( mieter, None, model.getSelectedYear(), monatidx, mobj_id )
        tv = BaseTableView()
        tv.setModel( eamodel )
        dlg = OkCancelDialog()
...
class AbschlagController( MtlEinAusController ):
    ...
    def getEinzelzahlungenModelMonat( self, debikredi: str, sab_id:int, jahr: int, monthIdx: int, mobj_id:str=None ) -> EinAusTableModel:
        ...
        keys = ("master_name", "mobj_id", "debi_kredi",  "leistung", "jahr", "monat", "betrag", "buchungstext", "write_time")
        headers = ("Haus", "Wohnung", "Kreditor",  "Leistung", "Jahr", "Monat", "Betrag", "Buchungstext", "write_time")
        eatm.setKeyHeaderMappings2( keys, headers )
        return eatm
