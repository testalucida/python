...
class MainController( QObject ):
    ...
    def createMainWindow( self ) -> MainWindow:
        ...
        self._mainWin.export_database.connect( self.onExportDatabase )
        ...
        try:
            ...
            box = ErrorBox( "Exception in MainController.createMainWindow()", str(ex) )
            box.exec_()

        ## see: https://stackoverflow.com/questions/68037950/module-pyqt6-qtwidgets-has-no-attribute-qdesktopwidget
        screen:QScreen = self._mainWin.screen()
    ...
    def onShowDividends( self, period:Period ):
        ...
        try:
            ...
            box = ErrorBox("Exception in MainController.onShowDividends()", str(ex) )
            box.exec_()
            return
    ...
    def onExportDatabase( self ):
        try:
            self._logic.exportDatabase()
            box = InfoBox( "Database uploaded", "Upload database was successful.", "" )
            box.exec_()
        except Exception as ex:
            box = ErrorBox( "Upload database failed", str( ex ),
                            "Transfer database manually." )
            box.exec_()
    ...
    def onPeriodIntervalChanged( self, period:Period, interval:Interval ):
        ...
        try:
            ...
            box = ErrorBox( "Exception in MainController.onPeriodIntervalChanged",
                            "Exception aufgetreten beim Call von InvestMonitorLogic.provideTickerHistories():\n",
                            str( ex ) )
            box.exec_()
            return
        self._summeDividendPaid = 0
    ...
    def _setSortKeyAndDirection( self, x:XDepotPosition, order:InfoPanelOrder ) -> str:
        ...
        if order == InfoPanelOrder.Wkn:
            ...
            self._sortDirection = SortDirection.ASC
        elif order == InfoPanelOrder.Gattung:
            if x.gattung == "Anleihe": sortfield = 0
            elif x.gattung == "Aktie": sortfield = 1
