...
class InfoPanel( QFrame ):
    ...
    def _createGui( self ):
        ...
        r += 1
        c = 0
        l.addWidget( BaseLabel( "1./Ltz. Kauf" ), r, c )
        c = 1
        lay = QHBoxLayout()
        font = QFont( "Ubuntu", 8 )
    ...
    def _setBorderstyle( self, color:str, thickness=None ):
        if not thickness:
            thickness = self._borderthicknessNormal
        self._borderstyle = "InfoPanel {border: " + thickness + " solid " + color + "; }"
        self.setStyleSheet( self._borderstyle )
    ...
    def setSelected( self, selected:bool=True ):
        if self._x.gattung == "Anleihe":
            bordercolor = self._bordercolorAnleihe
        elif self._x.gattung == "Geldmarkt":
            bordercolor = self._bordercolorGeldmarkt
        else:
            bordercolor = self._bordercolorFonds
        if selected:
            thickness = self._borderthicknessSelected
        else:
            thickness = self._borderthicknessNormal
        self._setBorderstyle( bordercolor, thickness )
