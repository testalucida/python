...
class InvestMonitorLogic:
    ...
    def provideTickerHistories( self, poslist:List[XDepotPosition], period:Period, interval:Interval ) -> List[XDepotPosition]:
        ...
        # closeDf: DataFrame = tickerHistories.iloc[:, tickerHistories.columns.get_level_values(1) == 'Close']
        # dividendsDf: DataFrame = tickerHistories.iloc[:, tickerHistories.columns.get_level_values(1) == 'Dividends']
        #closeDf: DataFrame = tickerHistories[SeriesName.Close.value]
        # dividendsDf: DataFrame = tickerHistories[SeriesName.Dividends.value]
        #closeDf = pd.DataFrame(tickerHistories, index=["Close",] )  #[SeriesName.Close.value] )
        #dividendsDf: DataFrame = pd.DataFrame( tickerHistories, index=[SeriesName.Dividends.value] )
        closeData: DataFrame or Series = tickerHistories["Close"]
        dividendsData: DataFrame or Series = tickerHistories["Dividends"]
        for deppos in poslist:
            self._provideOrderData( deppos )
            if len( poslist ) > 1:
                # closeData ist vom Typ DataFrame
                closeHist: Series = closeData[deppos.ticker].squeeze()
                dividends: Series = dividendsData[deppos.ticker].squeeze()
            else:
                # closeData ist vom Typ Series
                closeHist: Series = closeData
                dividends: Series = dividendsData
            # try:
    ...
    def getPaidDividendsTableModel( self, period:Period ) -> SumTableModel:
        ...
        dividends: DataFrame or Series = histlist[SeriesName.Dividends.value]
        #dividends:DataFrame = histlist.iloc[:, histlist.columns.get_level_values(1) == "Dividends"]
    ...
    def getSumDividendsCurrentYear( self, wkn_ticker_list:List[Dict], allOrders:List[XDelta] ) -> int:
        ...
        try:
            ...
            raise ex
        dividends:DataFrame or Series = histlist["Dividends"]
        # dividends: DataFrame = histlist.iloc[:, histlist.columns.get_level_values(1) == 'Dividends']
        sum_dividends = 0
        for ticker in tickers:
            if len( tickers ) > 1:
                # dividends ist vom Typ DataFrame
                divs = dividends[ticker].squeeze()
            else:
                # dividends ist vom Typ Series, keine Umwandlung nötig
                divs = dividends
            currency = TickerHistory.getCurrency( ticker )
            if currency != "EUR":
                divs = self._convertSeries( divs, currency )
